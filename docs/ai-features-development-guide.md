...
## 7. Worked example: House-Style Code Generation (2026-07-25)

Grounds the `ti` and `developer` personas' generated TI processes, rules, and
feeders in the organization's own coding standards, instead of generic TM1
templates. Three small, additive pieces, each independently mergeable:

1. **`search_knowledge_base` tool** (`src/ai/tools/knowledge.py`) — runs the
   same vector search the Knowledge Base's Ask box already uses
   (`knowledge.read` permission). Given to the `ti` and `developer` personas
   only. Degrades gracefully (empty result, not an error) if embeddings are
   unavailable — a retrieval failure must never abort a code-generation
   request. The service import is lazy inside `execute()`, since
   `src.knowledge.service` imports the AI orchestrator, which imports this
   tool registry at load — a naive top-level import would be circular.
2. **Prompt instructions** (`ti.yaml`/`developer.yaml`) — the persona MUST
   call `search_knowledge_base` before drafting any process/rule/feeder,
   adapt (never blindly copy) whatever house style it finds, infer a style
   from partial matches when there's no exact reference, and only fall back
   to generic TM1 practice when the Knowledge Base holds no organizational
   standards at all — saying so plainly either way. Includes an explicit,
   valid-TM1-only comment rule (each comment on its own line, `#`-prefixed,
   immediately above the code — never trailing, never inside a statement or
   parameter list) and a forbidden-output list (placeholder comments,
   unrequested TODOs, AI meta-notes like "Generated by AI"/"Example
   only"/"Pseudo code"). This is a **prompt-only** change — no code path
   changes, so it's essentially zero-risk to merge.
3. **Datasource/variable/parameter support** (`change_service.py`'s
   `_build_process`, `propose_process_update`'s schema) — without this, a
   generated process that reads a file (the most common real TI pattern)
   couldn't compile: its source columns and parameters had nowhere to be
   declared. `_apply_datasource`/`_apply_variables`/`_apply_parameters` set
   these on the TM1py `Process` object before the existing compile-dry-run
   step runs, so validation errors surface the same way they already did.

**How to verify this actually works, not just that it loads**: the failure
mode isn't a crash — a retrieval or prompt-compliance failure degrades
silently to generic output. Loading cleanly (tool registered, both personas
resolve) is necessary but not sufficient. Confirm live: pick the `ti` agent
in chat, ask it to draft a process against a cube with real Knowledge Base
coverage, and check the Tool Execution Timeline for an actual
`search_knowledge_base` call — then check the generated code cites which
standard it followed. If the tool never fires despite the "MUST" instruction,
that's the signal the prompt-based approach isn't reliable enough and code
generation needs the standards **injected** into the system prompt
unconditionally instead of left to the model's own tool-calling judgment.

---

## 8. Known dev-environment gotcha: the orphaned-socket bug
